...
class BinaryDbReader(object):
    ...
    def __init__(self, mode=None, batch_size=1, shuffle=True, use_wrist_coord=True, sigma=25.0, hand_crop=False,
        ...
        if mode == 'training':
            ...
            assert 0, "Unknown dataset mode."  # assert  用于判断一个表达式，在表达式条件为 false 的时候触发异常
    ...
    def get(self):
        ...
        # 计算每个记录的大小（这列出了DB中包含的内容以及占用的字节数）
        record_bytes = 2

        encoding_bytes = 4  # 四个字节
        kp_xyz_entries = 3 * self.num_kp  # 每个关键点有3个数据
        record_bytes += encoding_bytes*kp_xyz_entries  # x,y,z总的长度

        encoding_bytes = 4
        kp_uv_entries = 2 * self.num_kp
        record_bytes += encoding_bytes*kp_uv_entries  # 计算像素坐标的总长度+x,y,z

        cam_matrix_entries = 9
        record_bytes += encoding_bytes*cam_matrix_entries  # 再加上相机参数
        ...
        """ READ DATA ITEMS"""
        # Start reader，# 生成文件阅读器Reader，设置每次从文件中读取固定长度（record_bytes）的字符串记录Record
        reader = tf.FixedLengthRecordReader(header_bytes=0, record_bytes=record_bytes)
        # tf.train.string_input_producer回了一个包含路径和标签的列表，并将文件名和对应的标签放入文件名对列中，等待系统调用
        _, value = reader.read(tf.train.string_input_producer([self.path_to_db])) # 文件阅读器读取队列的值返回key,value
        ...
        # 1. Read keypoint xyz，tf.slice(inputs, begin, size, name)
        #  将x,y,z坐标提取出来，reshape成42*3的格式
        ...
        if not self.use_wrist_coord:  # tf.expand_dims(input, axis=None, name=None, dim=None)axis=0,标量
            #  在左右手的手腕节点和中指节点中间，即掌心处，增加一个标量？？？
        ...
        # 2. Read keypoint uv， tf.cast数据类型转换
        #  为什么转了int再转float????????????????????????????????????????????????????
        ...
        # calculate palm coord, 像素坐标与真是坐标一样处理
        ...
        # decode to uint8？？？？？？？？为啥要有这一步
        bytes_read += 2
        record_bytes_uint8 = tf.decode_raw(value, tf.uint8)

        # 4. Read imagetf.cast()函数的作用是执行 tensorflow 中张量数据类型转换，
        ...
        # cond_l为True所对应的输出为one_map,False为zero_map
        ...
        hand_side = tf.where(tf.greater(num_px_left_hand, num_px_right_hand),
                             ...
                             tf.constant(1, dtype=tf.int32))  # 创建一个常量， left hand = 0; right hand = 1 
                             # ？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？/
        ...
        data_dict['keypoint_xyz21_normed'] = kp_coord_xyz21_rel / index_root_bone_length 
        ...
        # tf.squeeze回相同类型的张量，并删除所有维度为1的维度。
